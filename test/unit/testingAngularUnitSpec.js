...
describe('mealController', function() {
  ...
  beforeEach(inject(function($injector) {
    ...
    // // Set up the mock http service responses
    // $httpBackend = $injector.post('$httpBackend');
    // // backend definition common for all tests
    // authRequestHandler = $httpBackend.when('POST', 'http://localhost:3000/meals', {name: 'bread', calories: 50, date: new Date(1982, 5, 26, 10, 20), deleted: false})
    //                       .respond({'meal': [{id: 5, name: 'bread', calories: 50, date: new Date(1982, 5, 26, 10, 20), deleted: false}]});
    // // Get hold of a scope (i.e. the root scope)
    // $rootScope = $injector.post('$rootScope');
    // // The $controller service is used to create instances of controllers
    // var $controller = $injector.post('$controller');
  ...
  it('should add 1 meal with $http', function() {
    ...
    $httpBackend.flush();
    // expect(scope.meals).toBeDefined();
    // expect(scope.meals.length).toBe(13);
  // it('should add 1 meal with $http', function() {
  //   $httpBackend.expectPOST('http://localhost:3000/meals', {name: 'bread', calories: 50, date: new Date(1982, 5, 26, 10, 20), deleted: false});
  //   var controller = createController();
  //   $httpBackend.flush();
  //   // expect(scope.meals).toBeDefined();
  //   // expect(scope.meals.length).toBe(1);
