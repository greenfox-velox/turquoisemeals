...
myMealApp.controller('mealController', ['$scope', '$http', function($scope, $http) {
  $scope.removeMeal = function(meal) {
    ...
    $http.delete('https://localhost:3000/meals/' + meal.id).success(function() {});
  };
  ...
  $scope.addMeal = function() {
    ...
    clearInputFields();
    $http.post('https://localhost:3000/meals', mealToAdd).success(function(data) {
      $scope.meals[$scope.meals.length - 1].id = data.meal.id;
    });
  };

  $http.get('https://localhost:3000/meals').success(function(data) {
